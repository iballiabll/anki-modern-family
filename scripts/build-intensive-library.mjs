// Builds the 四级听力精读 library from the public CET-4 listening pages.
// Usage: node scripts/build-intensive-library.mjs [paperId ...]
...
const outputDir = path.join(root, "intensive-data");
const manifestPath = path.join(root, "intensive-papers.js");
const SOURCE_ROOT = "https://english-exam.lazynote.cn/cet4/sections/listening";
const COLLOCATION_PATH = path.join(root, "data", "cet4-collocations.txt");
...
async function buildPaper(paperId, collocations) {
  ...
  const data = {
    meta: {
      id: paperId,
      title: `${year} 年 ${month} 月四级听力第 ${set} 套`,
      subtitle:
  ...
  const output = `// Generated from the public CET4 listening study page.\n(window.IBALL_INTENSIVE_LIBRARY = window.IBALL_INTENSIVE_LIBRARY || {})[${JSON.stringify(
    paperId,
  ...
  return {
    ...
    file: `./intensive-data/${paperId}.js`,
    pieceCount: pieces.length,
    paragraphCount,
    grammarCount,
    phraseCount,
...
console.log(`Loaded ${collocations.length} CET-4 collocations.`);
...
const manifest = `// Generated by scripts/build-intensive-library.mjs\nwindow.IBALL_INTENSIVE_PAPERS = ${JSON.stringify(
  summary,
  null,
  2,
)};\n`;
